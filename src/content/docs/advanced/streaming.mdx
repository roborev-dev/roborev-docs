...
The daemon exposes a REST API on the configured `server_addr` (default: `http://127.0.0.1:7373`). An OpenAPI 3.1.0 spec is available at `/openapi.json` for client generation and integration tooling.
...
curl --unix-socket /tmp/roborev-$(id -u)/daemon.sock http://localhost/openapi.json
```
...
All endpoints except `/api/job/output` have typed request/response schemas in the OpenAPI spec. `/api/job/output` uses streaming NDJSON and is registered as a plain handler outside the schema.
