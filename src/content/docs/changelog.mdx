...
- `--min-severity` flag for `roborev fix` and `roborev refine` to limit fixes to findings at or above a chosen severity (`low`, `medium`, `high`, `critical`).

**Improvements**

- Review sessions on the current branch are reused more safely, keeping review workflows continuous without stale session conflicts.
- Copied reviews now include review comments.
- Agent tool-call narration is removed from review output for cleaner results.
- Daemon status details are hidden from review views to keep the focus on review content.

**Bug fixes**

- Fix `roborev compact` using the wrong branch inside git worktrees.
