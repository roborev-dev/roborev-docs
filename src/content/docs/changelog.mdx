...
- Renamed `addressed`/`unaddressed` to `closed`/`open` across CLI, TUI, and API. The `roborev close` command replaces `roborev address`, and `roborev fix --open` replaces `--unaddressed`. Old names are still accepted as aliases.
