...
- `reuse_review_session` config option (global and per repo) to resume prior agent sessions on the same branch. When enabled, roborev looks for a recent session matching the repo, branch, agent, and review type, and reuses it if its reviewed commit is still a recent ancestor of the branch tip. `reuse_review_session_lookback` controls how many candidates to consider (default: unlimited).
