...
- Bundled systemd service and socket unit files for Linux daemon deployments. The service uses `Type=notify` for readiness signaling. Socket activation is supported for on-demand daemon startup. See [Keeping the Daemon Running](/integrations/github/#keeping-the-daemon-running).
