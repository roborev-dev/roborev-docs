...
- OpenAPI spec for the daemon REST API, served at `/openapi.json`. All endpoints use typed request/response schemas (OpenAPI 3.1.0 via Huma), enabling client generation and integration tooling. Endpoint discovery and schema inspection are available without separate documentation. See [Event Streaming](/advanced/streaming/#daemon-api).
