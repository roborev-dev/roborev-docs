...
Base keys (`review_model`, `refine_agent`, `fix_agent`) set the default for each workflow. Level-specific keys override for that reasoning level:
- `{workflow}_model_{level}` or `{workflow}_agent_{level}`
- `workflow` is `review`, `refine`, or `fix`
...
Template variables: `{job_id}`, `{repo}`, `{repo_name}`, `{sha}`, `{agent}`, `{verdict}`, `{error}`
