...
`roborev daemon start` runs the daemon in the background and is sufficient for most users. It won't survive a reboot, so if you need the daemon to start automatically on boot, set up a system service.
