...
The daemon starts automatically when you run `roborev init` or any command that needs it, and stays running in the background. This is sufficient for most users. If you need the daemon to restart automatically after a reboot, set up a system service.
