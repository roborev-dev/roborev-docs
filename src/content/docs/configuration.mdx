...
| `reuse_review_session` | bool | Resume prior agent sessions on the same branch when possible |
| `reuse_review_session_lookback` | int | Maximum number of recent session candidates to consider (default: unlimited) |
...
| `reuse_review_session` | bool | false | Resume prior agent sessions on the same branch | Yes |
