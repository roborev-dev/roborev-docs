...
```toml
default_agent = "codex"
default_model = "gpt-4.1"  # Model to use across all agents
...
The `default_model` setting specifies which model agents should use. The model format varies by agent:
...
1. CLI flag (`--model` / `-m`)
2. Per-repo config (`.roborev.toml`)
3. Global config (`~/.roborev/config.toml`)
4. Agent default
