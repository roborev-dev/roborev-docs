...
### Using the Built-in ACP Dispatcher

The simplest setup uses the `acp-agent` dispatcher with the `ROBOREV_ACP_ADAPTER` environment variable:
...
```bash
export ROBOREV_ACP_ADAPTER=claude
roborev review HEAD
```

### Pointing Directly at a Wrapper
...
Use `ROBOREV_ACP_ADAPTER_COMMAND` for a fully custom binary path:

```bash
export ROBOREV_ACP_ADAPTER_COMMAND=/opt/agents/my-custom-acp
roborev review HEAD
```

### Verifying the Setup

Check that roborev can find and communicate with the ACP agent:

```bash
roborev check-agents --agent acp
```
...
Once configured, the agent appears in the auto-detection fallback order and can be selected with `--agent my-agent`.
...
- **Path validation**: All file access is validated against the repository root using symlink-aware path resolution. Agents cannot read or write outside the repo.
...
- [Custom Tasks & Agentic Mode](/guides/custom-tasks/): Review vs agentic modes
