...
`roborev fix` processes unaddressed reviews one at a time. As it commits fixes, the daemon automatically reviews the new commits — and if those new reviews fail, `roborev fix` picks them up and addresses them too. This means a single `roborev fix` invocation can work through a chain of analysis findings and their follow-on reviews without manual intervention.
