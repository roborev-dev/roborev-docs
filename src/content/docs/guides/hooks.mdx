...
[[hooks]]
event = "review.*"
command = "echo {repo_name} {sha} {verdict} {error} >> ~/roborev-events.log"
```
...
For both failures and findings:

```toml
[[hooks]]
event = "review.*"
command = """test {verdict} = F -o -n {error} && curl -sf -X POST \
  -H 'Content-Type: application/json' \
  -d '{"text":"roborev: {repo_name} ({sha}) needs attention. Run `roborev show {job_id}`."}' \
  $SLACK_WEBHOOK_URL"""
```
