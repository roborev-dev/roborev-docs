...
| `type` | string | Built-in hook type: `"beads"` or `"webhook"`. Empty or `"command"` runs `command` |
...
The webhook sends a JSON POST with a 5-second timeout. The payload includes all event fields (`type`, `job_id`, `repo`, `repo_name`, `sha`, `agent`, `verdict`, `findings`, `error`). The `url` field is treated as sensitive and is masked in `roborev config list` output. Webhook URLs are redacted in daemon logs (only the scheme and host are shown).
