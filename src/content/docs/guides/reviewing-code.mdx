...
Use `--type` to change what the reviewer focuses on:

```bash
roborev review --type security      # Security-focused review
roborev review --type design        # Design document review
roborev review --branch --type security  # Security review of branch
```

| Type | Focus |
|------|-------|
| *(default)* | Bugs, security, testing gaps, regressions, code quality |
