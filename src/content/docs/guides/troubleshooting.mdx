...
```bash
cat "$(git rev-parse --path-format=absolute --git-path hooks)/post-commit"
```

This resolves `core.hooksPath` when set, falls back to `.git/hooks` otherwise, and produces an absolute path that works correctly from linked worktrees.
