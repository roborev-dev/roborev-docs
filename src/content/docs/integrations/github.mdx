...
3. **At least one AI agent** installed. The poller auto-detects installed agents in this order: `codex`, `claude-code`, `gemini`, `copilot`, `cursor`, `opencode`, `droid`, `kilo`, `kiro`. You can check what's available with:
   ```bash
...
When `[ci.reviews]` is set, `agents` and `review_types` are ignored. To disable reviews for a specific repo, set empty lists in the repo's `.roborev.toml`:
...
When `agents` is empty, the poller auto-detects the first available agent from: codex, claude-code, gemini, copilot, cursor, opencode, droid, kilo, kiro.
