...
| `Enter` | View review |
| `l` | View job log (persisted output) |
...
## Log View (Persisted Output)

Press `l` from the queue view, review view, or prompt view to open the log viewer. Unlike the tail view (which streams from the daemon's in-memory buffer), the log view reads from persisted log files on disk, so it works for any job whose logs have been saved, even after a daemon restart.
...
Press `F` on a completed review to launch a background fix job. A prompt modal opens where you can optionally provide custom instructions for the fix agent.
...
| `Enter` / `t` | View output (live for running, result for done, error for failed) |
| `p` | View patch diff |
