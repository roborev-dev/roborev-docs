...
The queue's Status column combines the previous Status and P/F (Pass/Fail) columns into a single field with color-coded values: Queued (yellow), Running (blue), Pass (green), Fail (red), Error (orange), Canceled (gray).
