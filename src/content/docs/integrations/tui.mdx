...
To start the TUI with closed items already hidden, set `hide_closed_by_default = true` in `~/.roborev/config.toml`. To auto-filter to the current repository on startup, set `auto_filter_repo = true`. See [Configuration](/configuration/) for details.
...
5. Press `a` to mark as closed
6. Use `h` to hide closed items
