...
To start the TUI with addressed items already hidden, set `hide_addressed_by_default = true` in `~/.roborev/config.toml`. See [Configuration](/configuration/) for details. Pressing `h` toggles the filter for the current session without changing the config.
